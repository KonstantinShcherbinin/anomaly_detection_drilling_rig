...
def walk_dir(path="C:/Users/shers/Desktop/SF/Буровая/data"):
    ...
    for dirname, _, filenames in os.walk(path):
        for file in filenames:
            ...
            data_f[['DEPT', 'HDTH']] = data_f[['DEPT', 'HDTH']].interpolate(method='ffill')  # Замена метода linear на ffill(04/05)
            data_f['DDEPT'] = data_f['DEPT'].diff()  # Новый признак изменения к предыдущему глубины долота
            if hole == '210ST2':
                data_f['RIG_STATE'] = data_f['RIG_STATE'].interpolate(method='ffill')
                data_f['BPOS'] = data_f['BPOS'].interpolate(method='ffill')  # Замена метода linear на ffill(04/05)
                data_f.loc[((data_f['BPOS'].isna()) & (data_f['RIG_STATE'] == 2)), 'BPOS'] = 1
                data_f['HKLD'] = data_f['HKLD'].interpolate(method='ffill')  # Замена метода linear на ffill(04/05)
                data_f['STOR'] = data_f['STOR'].interpolate(method='ffill')  # Замена метода linear на ffill(04/05)
            ...
            data_f['RIG_STATE'] = data_f['RIG_STATE'].fillna(method='ffill') #Замена метода bfill на ffill(04/05)
            ...
            data_f.loc[data_f['FLWI'].isna(), 'FLWI'] = data_f['FLWI'].interpolate(method='ffill')  # Замена метода linear на ffill(04/05)
            data_f.loc[((data_f['RPM'].isna()) & (data_f['RIG_STATE'].isin(RPM_0_RIG))), 'RPM'] = 0
            data_f.loc[data_f['RPM'].isna(), 'RPM'] = data_f['RPM'].interpolate(method='ffill')  # Замена метода linear на ffill(04/05)
            data_f['SPPA'] = data_f['SPPA'].interpolate(method='ffill')  # Замена метода linear на ffill(04/05)
            data_f['ECD'] = data_f['ECD'].fillna(method='ffill')

            data_f['DLS'] = data_f['DLS'].fillna(method='ffill')

            if 'INCL' in data_f.columns:
                data_f[['INCL', 'AZIM']] = data_f[['INCL', 'AZIM']].interpolate(method='ffill')
        ...
        data['F_all'] = data['F_all'].fillna(method='ffill')
